        ...
        while True:
            for i in range(0,numofsymbols):
                ...
                try:
                    ...
                    result=db['StockData'].insert_one( { 'company_symbol' : company_symbol[i], 'company_name': company_name[i],'price': x, 'Exchange':'MongoDB', 'tx_time': txtime.strftime('%Y-%m-%dT%H:%M:%SZ')}) # because mysql stores strings txtime})
